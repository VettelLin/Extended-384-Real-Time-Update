...
{
	m_swtBtPlate.Create(_T("孔板模式"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_SAMSETDLG_SWTBT_PLATE);
	m_swtBtChart.Create(_T("表格模式"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_SAMSETDLG_SWTBT_CHART);
}
...
void CSamSetDlg::InitLst()
{
	...
	m_lstSamInfo.InsertColumn(0, _T("序号"));
	m_lstSamInfo.InsertColumn(1, _T("反应孔"));
	m_lstSamInfo.InsertColumn(2, _T("样品类型"));
	m_lstSamInfo.InsertColumn(3, _T("样本名称"));
	m_lstSamInfo.InsertColumn(4, _T("样本ID"));
	m_lstSamInfo.InsertColumn(5, _T("标准品浓度"));
	m_lstSamInfo.InsertColumn(6,_T("管名"));
	m_lstSamInfo.InsertColumn(7, _T("患者姓名"));
	m_lstSamInfo.InsertColumn(8, _T("性别"));
	m_lstSamInfo.InsertColumn(9, _T("年龄"));
	m_lstSamInfo.InsertColumn(10, _T("采样日期"));
	m_lstSamInfo.InsertColumn(11, _T("送检医生"));
	m_lstSamInfo.InsertColumn(12, _T("送检科室"));
	m_lstSamInfo.InsertColumn(13, _T("病历号"));
	m_lstSamInfo.InsertColumn(14, _T("病床号"));
	m_lstSamInfo.InsertColumn(15, _T("住院号"));
	m_lstSamInfo.InsertColumn(16, _T("门诊号"));
	m_lstSamInfo.InsertColumn(17, _T("备注"));
...
void CSamSetDlg::InitSamPlateDlg()
{
	...
	m_samPlateDlg.SetFixedRowHeightColumnWidth(88 * m_dVertRatio,108 * m_dHoriRatio);
	m_samPlateDlg.SetRectWidth(40 * m_dHoriRatio);
	m_samPlateDlg.SetRectHeight(20 * m_dVertRatio);

#ifdef SAMPLATE_16SITE
	m_samPlateDlg.SetInterval(10 * m_dHoriRatio,16 * m_dVertRatio);
#else
	m_samPlateDlg.SetInterval(4 * m_dHoriRatio,9 * m_dVertRatio);
#endif
...
void CSamSetDlg::SetCtrlPos()
{
	...
	int iYStart = 28 * m_dHoriRatio + (m_iSwitchBtBkH - m_iSwitchBtH) / 2;
	...
	int iTempInt = rect.Height() - 84 * m_dVertRatio - 8 * m_dVertRatio - 1;

	m_rcManage.AddRectToDelete(32 * m_dHoriRatio,84 * m_dVertRatio,1344 * m_dHoriRatio,iTempInt);
	m_samPlateDlg.SetWindowPos(this,32 * m_dHoriRatio,84 * m_dVertRatio,1344 * m_dHoriRatio,746 * m_dVertRatio,SWP_NOZORDER);
	m_lstSamInfo.SetWindowPos(this,32 * m_dHoriRatio,84 * m_dVertRatio,1344 * m_dHoriRatio,iTempInt,SWP_NOZORDER);

	int iTempInt2 = rect.Height() - 12 * m_dVertRatio - 8 * m_dVertRatio - 1;
...
void CSamSetDlg::SetPlateSelectedSiteSamInfo(EITEMINFOTYPE eItemInfoType,CString strData,int intData /*= 0*/,double dData /*= 0*/)
{
	...
	CTubeInfo tubeInfo;
	if(ITEMSAMTYPE == eItemInfoType)	//判断修改之前的样本类型是否存在空类型
	{
		for(int i = 0;i < m_vecCurSelSite.size();++i)
		{
			m_samPlateDlg.GetSamInfo(m_vecCurSelSite[i],tubeInfo);
		...
		/*if(CMySamPalteDlg::SampleTypeToText(SAMPLE_TYPE_NONE) == strData)
		{
		bFindSamTypeNone = TRUE;
		}*/
	}
	...
	for(int i = 0;i < m_vecCurSelSite.size();++i)
	{
		m_samPlateDlg.SetSamInfo(m_vecCurSelSite[i],eItemInfoType,strData,intData,dData);
	}
	...
	for(int i = 0;i < m_vecCurSelSite.size();++i)
	{
		...
		{
			if(m_vecCurSelSite[i] == m_vecPlateSamIndex[m])
			{
				index = m;
				bFind = TRUE;
				break;
