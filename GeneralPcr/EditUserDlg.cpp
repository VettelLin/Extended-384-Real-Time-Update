...
BEGIN_MESSAGE_MAP(CEditUserDlg, CDialogEx)
	...
	ON_WM_ERASEBKGND() 
	ON_WM_LBUTTONDOWN()
	ON_BN_CLICKED(ID_EDITUSERDLG_BT_SAVE,OnBnClickedSave)
	ON_BN_CLICKED(ID_EDITUSERDLG_BT_CANCEL,OnBnClickedCancel)
	ON_BN_CLICKED(ID_EDITUSERDLG_CKBT_GENERALUSER,OnBnClickedCkbtGeneralUser)
	ON_BN_CLICKED(ID_EDITUSERDLG_CKBT_ADMINUSER,OnBnClickedCkbtAdminUser)
END_MESSAGE_MAP()
...
void CEditUserDlg::CreateBt()
{
	m_btSave.Create(_T("保存"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_EDITUSERDLG_BT_SAVE);
	m_btCancel.Create(_T("取消"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_EDITUSERDLG_BT_CANCEL);
}

void CEditUserDlg::CreateCkBt()
{
	m_ckbtGeneralUser.Create(_T("普通用户"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_EDITUSERDLG_CKBT_GENERALUSER);
	m_ckbtAdminUser.Create(_T("管理员用户"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_EDITUSERDLG_CKBT_ADMINUSER);
}

void CEditUserDlg::CreateSt()
{

}

void CEditUserDlg::CreateEd()
{
	m_edUserName.Create(WS_CHILD | WS_TABSTOP | ES_AUTOHSCROLL, CRect(0,0,0,0), this,ID_EDITUSERDLG_ED_USERNAME);
	m_edMnemonic.Create(WS_CHILD | WS_TABSTOP | ES_AUTOHSCROLL, CRect(0,0,0,0), this,ID_EDITUSERDLG_ED_MNEMONIC);
	m_edCode.Create(WS_CHILD | WS_TABSTOP | ES_AUTOHSCROLL, CRect(0,0,0,0), this,ID_EDITUSERDLG_ED_CODE);
}

void CEditUserDlg::InitDlg()
{
	SetTextClr1(RGB(86,86,86));
	SetMyFont(m_stTxFont,16 * m_dTxRatio,FW_NORMAL,TYPE_PIXEL);
}

void CEditUserDlg::InitCtrl()
{
	InitBt();
	InitCkBt();
	InitSt();
...
void CEditUserDlg::InitCkBt()
{
	...
	if(m_userInfo.GetBeOrdinaryPermission())
	{
...
void CEditUserDlg::InitSt()
{

}

void CEditUserDlg::InitEd()
{
	InitOneEdit(&m_edUserName);
	InitOneEdit(&m_edMnemonic);
	InitOneEdit(&m_edCode);

	m_edUserName.SetText(m_userInfo.GetName(),FALSE,TRUE);
	m_edUserName.SetBeEditable(FALSE);

	m_edMnemonic.SetText(m_userInfo.GetMnemonic(),FALSE,TRUE);
	m_edCode.SetText(m_userInfo.GetPassword(),FALSE,TRUE);

	return;
}

void CEditUserDlg::SetCtrlPos()
{
	CRect rect;
	GetClientRect(&rect);
	...
	m_btSave.SetWindowPos(this,rcUseful.left + 156 * m_dHoriRatio,rcUseful.top + 346 * m_dVertRatio,90 * m_dHoriRatio,iH,SWP_NOZORDER);
	m_btCancel.SetWindowPos(this,rcUseful.left + 270 * m_dHoriRatio,rcUseful.top + 346 * m_dVertRatio,90 * m_dHoriRatio,iH,SWP_NOZORDER);
}

void CEditUserDlg::ShowCtrl()
{
...
int CEditUserDlg::InitOneEdit(CMyNewEdit* pEdit)
{
	if(NULL == pEdit)
	{
		return -1;
...
BOOL CEditUserDlg::IsUserInfoChanged()
{
	return !(m_newUserInfo == m_userInfo);
}

int CEditUserDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL CEditUserDlg::OnEraseBkgnd(CDC* pDC)
{
	CRect rect;
	GetClientRect(&rect);
...
void CEditUserDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
#ifdef POPDLG_USESHADEBK
...
void CEditUserDlg::OnBnClickedSave()
{
	m_newUserInfo = m_userInfo;
	...
	m_newUserInfo.SetPassword(strTemp);
	

	if(m_ckbtGeneralUser.GetCheckState())
	{
		m_newUserInfo.SetPermissionType(EPT_ORDINARY);
	}
	else
	{
		m_newUserInfo.SetPermissionType(EPT_ADMIN);
	}
	
	m_newUserInfo.SetMnemonic(m_edMnemonic.GetText());
...
void CEditUserDlg::OnBnClickedCkbtGeneralUser()
{
...
void CEditUserDlg::OnBnClickedCkbtAdminUser()
{
	if(NULL != m_pCkbtCurUserType->GetSafeHwnd())
	{
		m_pCkbtCurUserType->SetBtChecked(!m_pCkbtCurUserType->GetCheckState());
