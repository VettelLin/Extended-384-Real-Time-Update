...
{
	m_btInsertSegment.Create(_T("插入程序段"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_PROCSETDLG_BT_INSERTSEGMENT);
	m_btInsertStep.Create(_T("插入步骤"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_PROCSETDLG_BT_INSERTSTEP);
	m_btEditStep.Create(_T("编辑步骤"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_PROCSETDLG_BT_EDITSTEP);
	m_btEditSegment.Create(_T("编辑程序段"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_PROCSETDLG_BT_EDITSEGMENT);
	m_btDelete.Create(_T("删除"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_PROCSETDLG_BT_DELETE);
	m_btAddCycle.Create(_T("添加循环数"),WS_CHILD | WS_CLIPSIBLINGS | BS_PUSHBUTTON,CRect(0,0,0,0),this,ID_PROCSETDLG_BT_ADDCYCLE);
}
...
void CProcSetDlg::InitSamPlateDlg()
{
	...
	m_samPlateDlg.SetInterval(7 * m_dHoriRatio,9 * m_dVertRatio);
...
int CProcSetDlg::SetPlateSelectedSiteSamInfo(CTubeChanEditInfo tubeChanEditInfo)
{
	...
	for(int i = 0;i < vecSiteToChanged.size();++i)
	{
		...
		//染料信息这块使用哪种模式，动态添加、删除 还是 页面产生时就已经固定
...
int CProcSetDlg::SetEditMode(EEXPEDITMODE eExpEditMode)  
{
	...
	if(EEEM_IVD == m_eExpEditMode)	//启动时是IVD模式
	{
