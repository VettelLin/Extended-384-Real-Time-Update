...
int CMySamPalteDlg::GetRowTag(int iRowId,CString& strTag)
{
	...
	// Excel-style letters: A..Z, AA..AZ, BA..
	CString temp(_T(""));
	int n = iRowId;
	while (n >= 0)
	{
		TCHAR ch = TCHAR('A' + (n % 26));
		temp.Insert(0, ch);
		n = n / 26 - 1;
	}
	strTag = temp;
...
int CMySamPalteDlg::GetSiteTag(int iSiteIndex,CString& strTag)
{
	...
	// Excel-style row letters
	CString rowLetters(_T(""));
	int rowIdx = iSiteIndex / m_colNum;
	while (rowIdx >= 0)
	{
		TCHAR ch = TCHAR('A' + (rowIdx % 26));
		rowLetters.Insert(0, ch);
		rowIdx = rowIdx / 26 - 1;
	}
	strTag.Format(_T("%s%d"), (LPCTSTR)rowLetters, iSiteIndex % m_colNum + 1);
	return 0;
}
...
int CMySamPalteDlg::GetSiteTag(int iRowId,int iColId,CString& strTag)
{
	...
	strTag.Format(_T("%s%d"), (LPCTSTR)rowLetters, iColId + 1);
	return 0;
}
