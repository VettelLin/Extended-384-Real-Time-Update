...
IMPLEMENT_DYNAMIC(CMySelWnd, CDialogEx)
...
BEGIN_MESSAGE_MAP(CMySelWnd, CDialogEx)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_SIZE()
	ON_WM_RBUTTONDOWN()
...
BOOL CMySelWnd::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}

void CMySelWnd::OnSize(UINT nType, int cx, int cy)
{
...
void CMySelWnd::OnRButtonDown(UINT nFlags, CPoint point)
{
	CRect rect;
	GetClientRect(&rect);
...
void CMySelWnd::OnLButtonDown(UINT nFlags, CPoint point)
{
	CRect rect;
	GetClientRect(&rect);
	if(rect.PtInRect(point))
...
void CMySelWnd::OnLButtonUp(UINT nFlags, CPoint point)
{
...
void CMySelWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	{
		CRect rect;
		GetClientRect(&rect);
...
BOOL CMySelWnd::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
void CMySelWnd::OnKillFocus(CWnd* pNewWnd)
{
	if(IsWindowVisible())
	{
		if(NULL != m_pParent->GetSafeHwnd())
		{
			m_pParent->PostMessage(WM_HIDESONCTRL,pNewWnd == m_pParent,0);
		}
	}
...
void CMySelWnd::OnShowWindow(BOOL bShow,UINT nStatus)
{
...
int CMySelWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
int CMySelWnd::DrawBody(CDC* pDC)
{
	if(NULL == pDC)
	{
		return -1;
...
int CMySelWnd::DrawVertScroll(CDC* pDC)
{
	if(NULL == pDC)
	{
		return -1;
...
int CMySelWnd::CalcRowToShow(int newStartRow)
{
...
int CMySelWnd::CalcTotalRowHeight()
{
	m_totalRowH = 0;
...
int CMySelWnd::GetHeight(int rowStart,int rowEnd)
{
...
int CMySelWnd::GetRowStartDrawInMemoryVertPixel(int rowId)
{
...
void CMySelWnd::ProcessVertSrcoll(UINT nFlags, CPoint point)
{
	if(m_vertScroll.m_bSliderDragMove)
	{
...
int CMySelWnd::ScrollRow(int pxVertShift)
{
	if(m_rowNum <= 0)
	{
		return -1;
...
int CMySelWnd::ScrollRowToFixPos(int pxFixPos)
{
	if(m_rowNum <= 0)
	{
		return -1;
...
CMySelWndDotPosInfo CMySelWnd::CalcPtPos(CPoint pt)
{
...
int CMySelWnd::SetRowSingleSelected(int rowId,BOOL bRedraw /*= FALSE*/)
{
	if(rowId < 0)
	{
		return -1;
...
int CMySelWnd::GetRowString(int rowId,CString& strText)
{
	strText = _T("");
	if(rowId < 0)
	{
...
int CMySelWnd::SetItemText(int iIndex,CString strText)
{
	if(iIndex < 0)
	{
		return -1;
	...
	m_lnkItemForSelInfo[iIndex]->m_strText = strText;
	return 0;
}

int CMySelWnd::InsertRow(int rowId,CString strText,BOOL bRedraw /*= TRUE*/)
{
	if(rowId < 0)
	{
		return -1;
...
int CMySelWnd::InsertRow(CString strText,BOOL bRedraw /*= TRUE*/)
{
	int rowId = m_rowNum;
...
int CMySelWnd::DeleteRow(int rowId,BOOL bRedraw /*= TRUE*/)
{
	if(rowId < 0)
	{
		return -1;
...
int CMySelWnd::DeleteAll(BOOL bRedraw /*= TRUE*/)
{
...
int CMySelWnd::SetSelectedRow(CString strSelectedRow,BOOL bRedraw /*= FALSE*/)
{
	for(int i = 0;i < m_rowNum;++i)
	{
...
int CMySelWnd::SetRowSelected(CString strSelectedRow,BOOL bRedraw /*= FALSE*/)
{
	for(int i = 0;i < m_rowNum;++i)
	{
		if(strSelectedRow == m_lnkItemForSelInfo[i]->m_strText)
...
int CMySelWnd::GetFirstSelectedRow(CString strSelectedRow)
{
	for(int i = 0;i < m_rowNum;++i)
	{
		if(strSelectedRow == m_lnkItemForSelInfo[i]->m_strText)
...
void CMySelWnd::SetBeAllowSameItem(BOOL bAllowSamItem)
{
	m_bAllowSameItem = bAllowSamItem;
}

BOOL CMySelWnd::GetBeAllowSameItem()
{
	return m_bAllowSameItem;
}

int CMySelWnd::InitVertScroll()
{
