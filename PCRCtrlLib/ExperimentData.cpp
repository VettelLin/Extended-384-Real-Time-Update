...
BOOL CExperimentData::SaveFile(CString strFilePath, BOOL bTemplate)
{
	...
	if (!bTemplate) // 如果是保存模板，则不改变当前文件状态
	{
		if (m_nStatus != STATUS_USING) // 实验过程中保存文件不需要改变状态
		{
			if (HasRunData())
			{
...
// 根据试剂项目信息设置孔板属性，如果输入为空，则使用成员项目信息
...
BOOL CExperimentData::ProcessAmplifyFluData(int nFlag /*= 5*/)
{
	...
	for(int i = 0;i < iTubeCount;++i)
	{
		...
		if(nFlag >= 3 && !m_bExpRunning)
		{
			...
			m_pAmplifyData->FluDigitalFilter(pTubeInfo,&m_arrayFilter,m_pAlgorithm, 1); // 只进行中值滤波
		}
		...
		if(nFlag >= 3)
		{
			// 滤波
			if (m_bExpRunning)
				m_pAmplifyData->FluDigitalFilter(pTubeInfo,&m_arrayFilter,m_pAlgorithm); // 进行全部滤波
			else
				m_pAmplifyData->FluDigitalFilter(pTubeInfo,&m_arrayFilter,m_pAlgorithm, 2); // 只进行均值滤波
		}
...
void CExperimentData::GenerateCt(BOOL bCalcDefThre,eMethodCalculateCt nMethod, eDataType nDataType)
{
	...
	for(int iTubeIndex = 0;iTubeIndex < iTubeCount; ++iTubeIndex)
	{
		...
		for(int i = 0;i < iTargetCount;++i)
		{
			...
			//使用通道分析参数20220418
			pAmplifyPara = NULL;
			//使用基因名未找到对应分析参数时，使用通道号再次获取分析参数，防止手动阈值等信息丢失，添加于20220418，孙文郎
...
void CExperimentData::GenerateOneChannelCt(eMethodCalculateCt nMethod, eDataType nDataType,int iChanId)
{
	...
	for(int iTubeIndex = 0;iTubeIndex < iTubeCount;++iTubeIndex)
	{
		...
		for(int i = 0;i < iTargetCount;++i)
		{
			...
			//使用通道分析参数20220415
			pAmplifyPara = NULL;
			//使用基因名未找到对应分析参数时，使用通道号再次获取分析参数，防止手动阈值等信息丢失，添加于20220401，孙文郎
			if(NULL == pAmplifyPara)
			{
...
int CExperimentData::GenerateAutoSplineThresholdForChannel(eDataType nDataType,BOOL bAutoThreCopyToHandThre /*= FALSE*/)
{
	...
	for(int j = 0;j < m_iChannelCount;++j)
	{
		...
		if (pAmplifyPara->nThresholdType == SPLINE_THRESHOLD_AUTO)// 自动阈值时
 		{			
			if(iCtCount > 0)
			{
	...
	while(NULL != pSegNode)
	{
		...
		while(NULL != pStepNode)
		{
			if(pStepNode->data.GetBeCollectAll())
			{
				...
				{
					dTemStart = 40;	//如果熔曲步骤是程序第一个步骤，则默认起始温度是40℃，20220715，孙文郎
				}
...
void CExperimentData::SetStatus(eDataStatus nStatus)
{
	...
	if (m_nStatus == STATUS_SAVE_DATA && nStatus == STATUS_CHANGE) // 有实验数据时不允许修改孔板以外的信息
	{
		ASSERT(FALSE);
		return;
	}
...
int CExperimentData::InputSampleSetInfo(vector<eSampleType>* pVecSamType /*= NULL*/)
{
	...
	for (int iCol=5; iCol<13; iCol++)
	{
		...
		if(strRead == pLanguageInfo->GetText(66,LANGUAGE_CHS) || strRead == pLanguageInfo->GetText(66,LANGUAGE_ENG))// 病人姓名
		{
			iPatientCol = iCol;
		}
	...
	int iFirstRow = sheet->firstRow();	//第一行
	int iLastRow = sheet->lastRow();	//包含字符信息的最后一行的下一行（对应空白行）
	...
	//PS96导入样本信息时，需要判断下“文件中的样本类型和孔板中设置的样本信息是否一致，不一致报错”
	if(NULL != pVecSamType)
	{
	...
	for(int i = iFirstRow + 2;i < iLastRow;++i)
	{
		...
		{
			pTubeInfo->SetSampleType(SAMPLE_TYPE_UNKNOWN);	//有样本ID的都是待测样本
		}
#endif
		...
		if(bPS96Instru)
		{
			...
			if(iTesterCol != -1)
			{
				...
				pTubeInfo->SetProposer(m_strTester); // 送检医生
			}
...
BOOL CExperimentData::ExportSampleSetInfo(int iFlag,int iIdVersion /*= 0*/)
{
	...
	sheet->setCol(0,0,16);	//width和像素宽度1:9
	...
	if(0 == iFlag)
	{
		for(int i = 0;i < pPlateInfo->iVertCount;++i)
		{
			for(int j = 0;j < pPlateInfo->iHoriCount;++j)
			{
				...
				sheet->writeStr(iRow,++iCol,pTube->GetTubeLabel(GetColCount()));
