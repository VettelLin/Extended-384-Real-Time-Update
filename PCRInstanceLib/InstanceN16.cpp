...
CInstanceN16::CInstanceN16(CSerialPortPCR* pComPort, CString strVersion) : CInstancePCR(pComPort, strVersion)
{
}


CInstanceN16::~CInstanceN16(void)
{
}

BOOL CInstanceN16::Initialize()
{
	InitDevicePara();
	InitDeviceStatus();
	InitStructurePara();
...
int CInstanceN16::CalculateExperimentTotalTimeSec(CLinkList<CSegItemInfo>& lnkSegItem,BOOL bUseLid,int iVolume)
{
	//使用临时的 整板扫描时间,nQ16默认扫描时间是2.6秒
	double dScanTimeSec = N16_SCANSEC;
	...
	while(NULL != pSegNode)
	{
		...
		while(NULL != pStepNode)
		{
			...
			if(CHECK_ALL_POINT == stepItem.m_eChkType)
			{
				...
				{
					if(pStepNode->pPrevious->data.m_iDelay < m_iPreMeltMinSlpSec)
					{
						iPreDelay = m_iPreMeltMinSlpSec - pStepNode->pPrevious->data.m_iDelay;
					}
				}
				else if(NULL != pSegNode->pPrevious && pSegNode->pPrevious->data.m_lnkStepItemInfo.GetSize() > 0)
				{
					int iTempInt1 = pSegNode->pPrevious->data.m_lnkStepItemInfo.GetTail()->data.m_iDelay;
					if(iTempInt1 < m_iPreMeltMinSlpSec)
					{
						iPreDelay = m_iPreMeltMinSlpSec - iTempInt1;
					}
				}
				iExpTotalSec += iPreDelay;	
			}
			else
			{
				if(0 == iStepIndex)
...
int CInstanceN16::GetLidHeatNormalSecond()
{
	return N16_LIDHEATSEC;
}

BOOL CInstanceN16::WriteLidPreheatTemperaturePLCUsed(float fLidTem)
{
	int iLidTem = fLidTem;
...
BOOL CInstanceN16::ReadLidPreheatTemperaturePLCUsed(float& fLidPreheatTem)
{
...
void CInstanceN16::InitDevicePara()
{
...
void CInstanceN16::InitStructurePara()
{
...
BOOL CInstanceN16::ScanOnce(BOOL *pBeAskTop)
{
	BOOL bReturn = FALSE;
	BYTE cDirection = 1;
...
BOOL CInstanceN16::ReadAllFLUData(CUIntArray* pAryData,int iChanNum)
{
	if(NULL == pAryData)
	{
		return FALSE;
