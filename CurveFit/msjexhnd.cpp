...
TCHAR MSJExceptionHandler::m_szDmpFileName[MAX_PATH];
...
MSJExceptionHandler::MINIDUMPWRITEDUMP
									MSJExceptionHandler::_MiniDumpWriteDump = 0;
...
MSJExceptionHandler::MSJExceptionHandler( )
{
	...
	GetModuleFileName( 0, m_szDmpFileName, MAX_PATH );
	...
	// Look for the '.' before the "EXE" extension.  Replace the extension
	// with "DMP"
	pszDot = _tcsrchr( m_szDmpFileName, _T('.') );
	if ( pszDot )
	{
		pszDot++;   // Advance past the '.'
		if ( _tcslen(pszDot) >= 3 )
                                    ...
                                    PEXCEPTION_POINTERS pExceptionInfo )
{
	...
	MessageBox(NULL, _T("Beta Service 发生未知异常，给您带来的不便敬请谅解，请联系软件提供商！"), _T("Exception"), MB_OK); 
    /*if ( m_previousFilter )
    ...
    PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    _tprintf( _T("//=====================================================\n") );
	// print information about the system time
	SYSTEMTIME sys;   
	GetLocalTime( &sys );   
	_tprintf(_T("System Time: %4d/%02d/%02d %02d:%02d:%02d.%03d 星期%1d\n") ,sys.wYear,sys.wMonth,sys.wDay,sys.wHour,sys.wMinute, sys.wSecond,sys.wMilliseconds,sys.wDayOfWeek);
	...
	{
		HANDLE hDumpFile = CreateFile(m_szDmpFileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);  
		...
		_MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hDumpFile, mdt/*MiniDumpNormal*/, &dumpInfo, NULL, NULL);  
		CloseHandle(hDumpFile); 
	}
	else
	{
		_tprintf( _T("dbghelp.dll or its exported procs not found") );
	}

    if ( !InitImagehlpFunctions() )
    {
        //OutputDebugString(_T("IMAGEHLP.DLL or its exported procs not found"));
         _tprintf( _T("IMAGEHLP.DLL or its exported procs not found") );
        #ifdef _M_IX86  // Intel Only!
...
//=========================================================================
// Load dbghelp.dll and get the address of functions in it that we'll use 
//=========================================================================
BOOL MSJExceptionHandler::InitDbghelpFunctions( void )
{
